...
public class LowestPrice {
	...
	public int shopping(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
		...
		for (List<Integer> s : special) {
			...
			if (j == needs.size()) {
				List<List<Integer>> specialClone = new ArrayList<>();
				specialClone.addAll(special);
				specialClone.remove(s);
				res = Math.min(res, s.get(s.size() - 1) + shopping(price, specialClone, clone));
			}
		}
		return res;
	}
